...
export default function CreateListing() {
  ...
  useEffect(() => {
    ...
    // "useEffect" cannot call an async function directly. Therefore, to make "useEffect" call an async function, inside the function directly called by "useEffect" we must: 1. declare the async function; 2. call the async function.
    const fetchListing = async () => {
      const listingId = params.listingId; // Note that the path of this page is "/update-listing/:listingId".
  ...
  const handleImageSubmit = () => {
    if (files.length > 0 && files.length + formData.imageUrls.length < 7){
      ...
      // "Promise.all()" takes an array (or an iterable) of promises as an input and returns a single promise.
  ...
  // This function is similar to "handleFileUpload" function in "Profile.jsx".
  const storeImage = async file => {
    return new Promise((resolve, reject) => {
      ...
      uploadTask.on(
        ...
        // We must have this even if we don't need to know the upload progress, otherwise "uploadTask.on()" will return an error.
        snapshot => {
  ...
  const handleChange = e => {
    ...
    // Handle the change of "parking", "furnished", and "offer".
    ...
    // Handle the change of the rest attributes.
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      // In case no image has been uploaded.
      if (formData.imageUrls.length < 1){
        return setError('You must upload at least one image');
      }

      // In case the discounted price isn't lower than the regular price.
      if (+formData.regularPrice < +formData.discountedPrice){ // "+" converts the variable type into number.
